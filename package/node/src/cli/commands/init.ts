  ...
  try {
    ...
    if (!options.force) {
      try {
        ...
        console.error(
          "❌ vault.config.ts already exists. Use --force to overwrite.",
        );
        process.exit(1);
      } catch {
    ...
    console.log("✅ vault.config.ts created successfully!");
    ...
    console.error("❌ Failed to create vault.config.ts:", error);
    process.exit(1);
  }
}
